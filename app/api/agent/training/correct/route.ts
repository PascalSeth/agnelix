...
export async function POST(req: NextRequest) {
  ...
  const { surface, scenario, aiResponse, correction, feedback } = await req.json()
  ...
  const user = await prisma.user.findUnique({ where: { id: scopeId }, select: { playbookType: true } })
  const scope = user?.playbookType || "global"
  ...
  try {
    ...
    // Agency-owned lesson — userId set, so it only ever applies to this company
    ...
    console.error("[agent/training/correct] failed:", err)
