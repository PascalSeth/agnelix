...
export async function POST(req: NextRequest) {
  ...
  const { websiteUrl, companyName, localNeighbors, bypassCache } = await req.json()
  if (!websiteUrl) return NextResponse.json({ error: "websiteUrl required" }, { status: 400 })

  try {
    const contacts = await findContacts(websiteUrl, companyName ?? "", localNeighbors, bypassCache)
