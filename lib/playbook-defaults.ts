...
// Server-side playbook defaults. Seeded into the DB by prisma/seed.ts and used
// by the dashboard layout to merge with DB rows. Do NOT import this from
// client components — the whole point is keeping this data out of the bundle.
...
export const DEFAULT_PLAYBOOKS: PlaybookConfig[] = [
  {
...
/**
 * Merges DB playbook rows over the hardcoded defaults (DB wins per type),
 * normalising Json columns that may have been stored as strings.
 */
export function mergePlaybooksWithDefaults(dbPlaybooks: any[]): PlaybookConfig[] {
  return DEFAULT_PLAYBOOKS.map((def) => {
    const db = dbPlaybooks.find((p) => p.type === def.type)
    if (!db) return def
    return {
      ...db,
      targetVerticals: parseJsonField(db.targetVerticals, def.targetVerticals),
      platformOptions: parseJsonField(db.platformOptions, def.platformOptions),
      sequenceTemplates: parseJsonField(db.sequenceTemplates, def.sequenceTemplates),
      proposalTemplates: parseJsonField(db.proposalTemplates, def.proposalTemplates),
      reportMetrics: parseJsonField(db.reportMetrics, def.reportMetrics),
      reportTemplates: parseJsonField(db.reportTemplates, def.reportTemplates),
      portalTemplates: parseJsonField(db.portalTemplates, def.portalTemplates),
      portalSections: parseJsonField(db.portalSections, def.portalSections),
      toneOptions: parseJsonField(db.toneOptions, def.toneOptions),
      objectionHandlers: parseJsonField(db.objectionHandlers, def.objectionHandlers),
    } as PlaybookConfig
  })
}
